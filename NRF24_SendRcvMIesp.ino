 ...
 * derFrickler.net: Änderungen für TSOL-M800 hinzugefügt
...
----------------------------------------
*/
#define VERSION "V0.1.5_frickler"
...
#include "Sonne.h"
  ...
  #include "mqtt.h"
#else
 float PowGrid=0; //arduino has no mqtt, we just say grid is null
#endif
...
//---------------------------------------------------------------------------------------------------------------------
 ...
 uint8_t pipe;
 bool sts=false;
 //  DEBUG_OUT.println(F("RFRxPacket BEGIN"));
// Loop until RX buffer(s) contain no more packets.
  while (radio1.available(&pipe)) {
    ...
    if (!packetBuffer.full()) {
      ...
      if (!p) DEBUG_OUT.println("RFRxPacket packetbuffer getFront full");
      ...
      if (!p) DEBUG_OUT.println("RFRxPacket packetbuffer pushFront full");
      else sts = true;
      lostPacketCount = 0;
      ...
      if (lostPacketCount < 255){
         lostPacketCount++;
         DEBUG_OUT.println(F("RFRxPacket lost packet, full buffer"));
         }
      DEBUG_OUT.println(F("RFRxPacket full buffer"));
      sts = false;
      }
...
//---------------------------------------------------------------------------------------
  ...
  while (!radio1.begin()) {
    DEBUG_OUT.println(F("Radio hardware is not responding!!"));
    delay(1000);
    }
...
//---------------------------------------------------------------------------------------
  ...
  DEBUG_OUT.flush();
  DEBUG_OUT.println(F("....................."));
  DEBUG_OUT.println(F("DTU-Simulation for MI "));
  DEBUG_OUT.println(F("....................."));
  DEBUG_OUT.println(F("Begin Setup  wait...."));
  delay(2000);
  #ifdef ESP8266
    if (WITHWIFI){
      if(!SNIFFER){
          ...
          DEBUG_OUT.println(F("Setup clock ....."));
          setupClock();
          DEBUG_OUT.println(F("Setup mqtt ......"));
          setupMQTT();
          DEBUG_OUT.println(F("Setup www ......."));
          setupWebServer();
          setupUpdateByOTA();
          calcSunUpDown (getNow());
          istTag = isDayTime(0);
          DEBUG_OUT.printf ("Es ist "); DEBUG_OUT.println (istTag?"Tag":"Nacht");
  ...
  if (MI600) strcpy(MIWHAT,"MI-600/TSUN TSOL-M800");
  if (MI1500) strcpy(MIWHAT,"MI-1500");

  DEBUG_OUT.print(F("Microinverter is  "));DEBUG_OUT.print(MIWHAT);
  DEBUG_OUT.print(F("  starting "));DEBUG_OUT.println(STARTTIME);
  TxLimitSts=0;
//  DEBUG_OUT.print("TxLimitSts setup ---- ");DEBUG_OUT.println(TxLimitSts);
  DEBUG_OUT.print(F("Version  "));DEBUG_OUT.println(VERSION);
...
//----------------------------------------------------------------------------------------------------
  ...
  if (DEBUG_TX_DATA) {
      if (TxCH<10)  //z.B wenn CH 3 wird 03 ausgegeben
        DEBUG_OUT.print(F("Send... CH0"));
      else DEBUG_OUT.print(F("Send... CH"));
      DEBUG_OUT.print(TxCH);
      DEBUG_OUT.print(F(" "));
...
//----------------------------------------------------------------------------------------------------------------------
    switch (SerCmd){
        ...
        case 2:
          DEBUG_OUT.printf("\r\n\r\nVersion\t %s\r\n",VERSION);
          DEBUG_OUT.print("Start \t");DEBUG_OUT.println(STARTTIME);
  ...
  if (millis() >= UpdateTxMsgTick) {
    ...
    switch(telegram) {
      case 0:
        ...
        if ((TxLimitSts) && (abs (PowGrid) > TOLERANCE)) {        //Limitierung ????chek it again
          Cmd=0x51;
          DEBUG_OUT.printf("CMD:%03X CH:%i Sts:%4i Lm:%i\r\n",Cmd, TxCH,TxLimitSts,Limit);
...
//  }
  while (!packetBuffer.empty()) {
    ...
    if (CHECK_CRC) {
      ...
      if (((crc >> 8) != p->packet[payloadLen + 2]) || ((crc & 0xFF) != p->packet[payloadLen + 3])) {
        if (p->packetsLost > 0) {
          DEBUG_OUT.println(F("RFAnalyse CRC lost packets: ")); DEBUG_OUT.println(p->packetsLost);
          }
        //DEBUG_OUT.println(F("RFAnalyse CRC fail "));
...
//        radio1.flush_rx();// Flush buffer to drop the packet.
      ...
      if (lastCRC == crc) {
//
//        DEBUG_OUT.println(F("RFAnalyse last CRC "));
        packetBuffer.popBack();
//        packetBuffer.clear();//????????
//        radio1.flush_rx();// Flush buffer to drop the packet.
        continue;
    ...
    if (payloadLen == 0) {
      packetBuffer.popBack();
      DEBUG_OUT.println(F("RFAnalyse mysterious ack "));
      continue;
      }
...
//-------------------------------------------------------------------------------------------------
    ...
    if (millis() < UpdateZeroExpTick){//not overload the web&mqtt server
        DEBUG_OUT.println(F("ZeroExport timer: not yet !"));
        return; //not in first 60 sek
        }
    else UpdateZeroExpTick += ZEXPUPDTICK;
    if (!TxLimitSts){
        if (abs (PowGrid) > TOLERANCE) { // || (abs (Limit-OldLimit) > TOLERANCE )){//if change more than 15 watt
            DEBUG_OUT.print(F("Grid out of Tolerance "));DEBUG_OUT.println(PowGrid);
            if (PowGrid >0){ //Exporting is PLUS zu viel power
                Limit= PMI - PowGrid;  //327-(296)
                }
            else { //importing is minus zu wenig power
                Limit= PMI+ abs(PowGrid); //327+abs(-296)
                }
  ...
  PVcheck();
    ...
    if (WITHWIFI && (!SNIFFER)){
       ...
       if (millis() >= UpdateMqttTick){//not overload the web&mqtt server
        //Serial.println("UpdateMqttTick");
         istTag = isDayTime(0);
         UpdateMqttTick += IPServUPDATETICK;
         if (MQTT) mqttClient.poll();
         if (PVcheck()){
            //Serial.println("PVCheck");
             ...
             webserverHandle();
             if (MQTT && DataOK ){
               //DEBUG_OUT.println(F("Update Mqtt Services"));
               mqttClient.poll();
               ...
               SendMQTTMsg((String)INV_CONSUM_P, (String) (abs(PowGrid) + PMI));
               ...
               SendMQTTMsg((String)INFO, "NRF24");
                ...
                String("\"ConsumP\":")+String(abs(PowGrid) + PMI)+
                String("}}");

                mqttClient.beginMessage((String)TOPIC);
