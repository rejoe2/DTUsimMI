...
#ifndef __MI_SYSTEM_H__
#define __MI_SYSTEM_H__

#include "miInverter.h"
#ifndef NO_RADIO
#include "hmRadio.h"
#endif
...
class MiSystem {
    public:
        ...
        void setup() {
            DPRINTLN(DBG_VERBOSE, F("miSystem.h:setup"));
            Radio.setup(&BufCtrl);
        }

        INVERTERTYPE *addInverter(const char *name, uint64_t serial, uint16_t chMaxPwr[]) {
            DPRINTLN(DBG_VERBOSE, F("miSystem.h:addInverter"));
            ...
            if(p->serial.b[5] == 0x10) {
                switch(p->serial.b[4]) {
                    case 0x21: p->type = INV_TYPE_1CH; break; //seems to be more or less ok
        ...
        INVERTERTYPE *findInverter(uint8_t buf[]) {
            DPRINTLN(DBG_VERBOSE, F("miSystem.h:findInverter"));
            INVERTERTYPE *p;
        ...
        INVERTERTYPE *getInverterByPos(uint8_t pos) {
            DPRINTLN(DBG_VERBOSE, F("miSystem.h:getInverterByPos"));
        ...
        uint8_t getNumInverters(void) {
            DPRINTLN(DBG_VERBOSE, F("miSystem.h:getNumInverters"));
            return mNumInv;
        }
