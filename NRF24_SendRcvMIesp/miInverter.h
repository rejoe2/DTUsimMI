...
#ifndef __MI_INVERTER_H__
#define __MI_INVERTER_H__
...
class Inverter {
    public:
        ...
        void init(void) {
            DPRINTLN(DBG_VERBOSE, F("miInverter.h:init"));
            getAssignment();
            toRadioId();
        ...
        uint8_t getPosByChFld(uint8_t channel, uint8_t fieldId) {
            DPRINTLN(DBG_VERBOSE, F("miInverter.h:getPosByChFld"));
            uint8_t pos = 0;
            for(; pos < listLen; pos++) {
        ...
        const char *getFieldName(uint8_t pos) {
            DPRINTLN(DBG_VERBOSE, F("miInverter.h:getFieldName"));
            return fields[assign[pos].fieldId];
        }

        const char *getUnit(uint8_t pos) {
            DPRINTLN(DBG_VERBOSE, F("miInverter.h:getUnit"));
            return units[assign[pos].unitId];
        }

        uint8_t getChannel(uint8_t pos) {
            DPRINTLN(DBG_VERBOSE, F("miInverter.h:getChannel"));
            return assign[pos].ch;
        }

        void addValue(uint8_t pos, uint8_t buf[]) {
            DPRINTLN(DBG_VERBOSE, F("miInverter.h:addValue"));
        ...
        RECORDTYPE getValue(uint8_t pos) {
            DPRINTLN(DBG_VERBOSE, F("miInverter.h:getValue"));
            return record[pos];
        }

        void doCalculations(void) {
            DPRINTLN(DBG_VERBOSE, F("miInverter.h:doCalculations"));
        ...
        bool isAvailable(uint32_t timestamp) {
            DPRINTLN(DBG_VERBOSE, F("miInverter.h:isAvailable"));
            return ((timestamp - ts) < INACT_THRES_SEC);
        }

        bool isProducing(uint32_t timestamp) {
            DPRINTLN(DBG_VERBOSE, F("miInverter.h:isProducing"));
        ...
        uint32_t getLastTs(void) {
            DPRINTLN(DBG_VERBOSE, F("miInverter.h:getLastTs"));
            return ts;
        }

    private:
        void toRadioId(void) {
            DPRINTLN(DBG_VERBOSE, F("miInverter.h:toRadioId"));
            radioId.u64  = 0ULL;
        ...
        void getAssignment(void) {
            DPRINTLN(DBG_VERBOSE, F("miInverter.h:getAssignment"));
...
template<class T=float>
static T calcYieldTotalCh0(Inverter<> *iv, uint8_t arg0) {
    DPRINTLN(DBG_VERBOSE, F("miInverter.h:calcYieldTotalCh0"));
    if(NULL != iv) {
        T yield = 0;
...
template<class T=float>
static T calcYieldDayCh0(Inverter<> *iv, uint8_t arg0) {
    DPRINTLN(DBG_VERBOSE, F("miInverter.h:calcYieldDayCh0"));
    if(NULL != iv) {
        T yield = 0;
        for(uint8_t i = 1; i <= iv->channels; i++) {
...
template<class T=float>
static T calcUdcCh(Inverter<> *iv, uint8_t arg0) {
    DPRINTLN(DBG_VERBOSE, F("miInverter.h:calcUdcCh"));
...
template<class T=float>
static T calcPowerDcCh0(Inverter<> *iv, uint8_t arg0) {
    DPRINTLN(DBG_VERBOSE, F("miInverter.h:calcPowerDcCh0"));
    if(NULL != iv) {
        T dcPower = 0;
        for(uint8_t i = 1; i <= iv->channels; i++) {
...
template<class T=float>
static T calcEffiencyCh0(Inverter<> *iv, uint8_t arg0) {
    DPRINTLN(DBG_VERBOSE, F("miInverter.h:calcEfficiencyCh0"));
    if(NULL != iv) {
...
template<class T=float>
static T calcIrradiation(Inverter<> *iv, uint8_t arg0) {
    DPRINTLN(DBG_VERBOSE, F("miInverter.h:calcIrradiation"));
    // arg0 = channel
    if(NULL != iv) {
