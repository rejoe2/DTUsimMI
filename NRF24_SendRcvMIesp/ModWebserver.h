...
void handleRoot() {
//---------------------------------------------------------------------------------------------------------
   String out = "<!DOCTYPE html><html><head><title>Hoylymoly</title><meta http-equiv='refresh' content='10':URL='";
    ...
    out += "<h2>Hoymiles Micro-Inverter "+ String(MIWHAT)+"</h2>";
    out += "<h4>"+(String)getDateStr(getNow())+" "+(String)getTimeStr(getNow())+"</h4>";

    out += "<table id='myT'>";
    out += "<tr><th>MI P</th><th>-Imp/Exp+</th><th>Limit</th><th>U AC</th><th>Freq</th><th>Temp</th></tr>";
    out += "<tr><td>"+String(PMI) +" W</td><td>"+String(P_DTSU)+" W</td><td>"+String(LIMIT)+" W</td><td>"+String(U_AC);
